...
$.wn.builder.formbuilder.controller.addControlToPlaceholder($placeholder,field.type,field.label?field.label:field.column,false,field.column).complete(function(){defer.resolve()})
return defer.promise()};}var allFields=$.when({})
$.each(fields,function(index,field){allFields=allFields.then(addField(field))});$.when(allFields).always($.wn.builder.indexController.hideStripeIndicatorProxy)}
...
var $masterTabPane=this.getMasterTabsActivePane(),$form=$masterTabPane.find('form'),$toolbar=$masterTabPane.find('div[data-control=table] div.toolbar'),$button=$('<a class="btn wn-icon-magic builder-custom-table-button" data-builder-command="modelList:cmdAddDatabaseColumns"></a>')
...
Builder.prototype.addMasterTab=function(data){this.masterTabsObj.addTab(data.tabTitle,data.tab,data.tabId,'wn-'+data.tabIcon)
